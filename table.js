...
function cookProgress(r){ const ck=checksFor(r.id), total=(r.steps||[]).length; if(!total) return null; let done=0,next=0; for(let i=0;i<total;i++){if(ck['s:'+r.id+':'+i])done++;else if(!next)next=i+1;} if(done<=0||done>=total) return null; return {done,total,next}; }
...
function route(){
  ...
  renderHome(h.startsWith('/saved')?'saved':'recipes');
}
...
function tImport(){
  ...
  store = obj; store.lang = store.lang||'en'; store.fb = store.fb||{}; store.theme=store.theme||'auto'; save(); applyLang(); applyTheme(); route();
}
...
function renderDetail(r){
  ...
  app.innerHTML = header() + `
    <div class="detail">
      <button class="back">${ICON_BACK}<span>${nt('back')}</span></button>
      ${detailNavigation(r)}
    ...
    <button class="cookbtn">${ICON_COOK}<span>${t('cook')}</span></button>${bottomNav('')}`;
  bindLang(app);
  app.querySelector('.back span').textContent=returnRoute==='/shop'?(store.lang==='tl'?'Bumalik sa bilihin':'Back to shopping'):returnRoute.startsWith('/saved')?(store.lang==='tl'?'Bumalik sa naka-save':'Back to saved'):nt('back');
  app.querySelector('.back').onclick=()=>nav(()=>go(returnRoute));
...
// Offer installed clients a fresh page without interrupting cooking or notes.
if('serviceWorker' in navigator){
  const hadController=!!navigator.serviceWorker.controller;
  let updating=false;
  navigator.serviceWorker.addEventListener('controllerchange',()=>{
    if(updating||!hadController)return; updating=true;
    const notice=document.createElement('button');notice.className='table-update';notice.textContent=store.lang==='tl'?'May bagong bersyon · Buksan':'Table has an update · Refresh';
    notice.onclick=()=>{const url=new URL(location.href);url.searchParams.set('v',Date.now());location.replace(url);};document.body.appendChild(notice);
  });
}
